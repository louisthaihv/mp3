	...
	{	$sql1=mysql_query("select * from baihathot");
		...
		if($_POST['ok'])
		{
			...
			if($file_type==true)
			{
				...
				$src = '/nhac'.$_FILES['upload']['name'];
				if(copy($source,$dest))
				{
					$sql="insert into baihat(tenbaihat,casy,theloai,duongdan) values('$tenbaihat','$casy','$theloai','$src')";
...
die('thai');
	if($_POST['sua'])	
	{
		...
		else
		{
			...
			$update= mysql_query("update baihathot set tenbaihat='$tenbaihat', casy='$casy', theloai='$theloai' where id IN ($s_id)");
